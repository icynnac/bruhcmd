            ...
            if (sender.hasPermission("bruhcmd.joe")) {
                if (args.length == 0) {
                    ...
                    scheduler.scheduleSyncDelayedTask(Main.instance, () -> {
                        ...
                        p.giveExp(30);
                        p.getInventory().addItem(new ItemStack(Material.getMaterial(Main.instance.getConfig().getString("configuration.other.reward-item").toUpperCase()), Main.instance.getConfig().getInt("configuration.other.reward-amount")));
                    }, 280);
                } else {
                    Player target = Bukkit.getPlayer(args[0]);
                    if (target == null) {
                        p.sendMessage(ChatColor.translateAlternateColorCodes('&', Main.instance.getConfig().getString("configuration.messages.player-not-found")));
                    } else {
